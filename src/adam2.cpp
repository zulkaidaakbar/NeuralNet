...
private:
	...
	static double randomWeight(void) { return 1.0*distribution(generator); }
...
double Neuron::eta = 0.005; // overall net learning rate
double Neuron::alpha = 0.0025; // momentum, multiplier of last deltaWeight, [0.0..n]
...
void Neuron::updateInputWeights(Layer &prevLayer)
{
	...
	for(unsigned n = 0; n < prevLayer.size(); ++n)
	{
		...
		neuron.m_outputWeights[m_myIndex].vdw = beta1 * neuron.m_outputWeights[m_myIndex].vdw + (1 - beta1) * neuron.getOutputVal()* m_gradient;
		neuron.m_outputWeights[m_myIndex].sdw = beta2 * neuron.m_outputWeights[m_myIndex].sdw + (1 - beta2) * pow(neuron.getOutputVal()* m_gradient, 2);
		double vdw_corrected = neuron.m_outputWeights[m_myIndex].vdw / (1-pow(beta1, epoch+1));
		double sdw_corrected = neuron.m_outputWeights[m_myIndex].sdw / (1-pow(beta2,epoch+1));
		neuron.m_outputWeights[m_myIndex].weight = neuron.m_outputWeights[m_myIndex].weight - eta*(vdw_corrected / (sqrt(sdw_corrected) + epsilon));
	}
}
...
Neuron::Neuron(unsigned numOutputs, unsigned myIndex)
{
	for(unsigned c = 0; c < numOutputs; ++c){
		...
		m_outputWeights.back().vdw = 0.0;
		m_outputWeights.back().sdw = 0.0;
	}
...
int main()
{
	TrainingData trainData("epoch_test8_more_set3.txt");
	//e.g., {3, 2, 1 }
